...
import { normalizeSettings } from '../../apps/pulse/domain/types';
import type { DriftEvent, DriftSession } from '../../apps/drift/domain/types';
...
function driftSessionToRow(userId: string, s: DriftSession) {
  return {
    id: s.id,
    user_id: userId,
...
function rowToDriftSession(r: Record<string, unknown>): DriftSession {
  const events = Array.isArray(r.events) ? (r.events as DriftEvent[]) : [];
  return {
    id: String(r.id),
    intention: String(r.intention ?? 'Stay with the work'),
    taskId: r.task_id ? String(r.task_id) : null,
    startedAt: String(r.started_at),
    endedAt: r.ended_at ? String(r.ended_at) : null,
    events,
...
/** Pull cloud Drift watches and merge with local (local wins on id). */
export async function pullAndMergeDriftSessions(
  userId: string,
  local: DriftSession[],
): Promise<DriftSession[]> {
  const sb = getSupabase();
  if (!sb || !isSupabaseConfigured() || userId === 'local-guest') {
    return local;
  }
  const { data, error } = await sb
    ...
    .limit(120);
  if (error) {
    console.warn('[sync] drift pull failed', error.message);
    return local;
  }
  const remote = (data ?? []).map((r) =>
    rowToDriftSession(r as Record<string, unknown>),
  );
...
/** Upsert Drift watches to Supabase. */
export async function pushDriftSessions(
  userId: string,
  ...
  const { error } = await sb
    .from('drift_sessions')
    .upsert(
      sessions.map((s) => driftSessionToRow(userId, s)),
      { onConflict: 'id' },
    );
  if (error) console.warn('[sync] drift push', error.message);
}

let driftPushTimer: ReturnType<typeof setTimeout> | null = null;

export function schedulePushDriftSessions(
  userId: string,
  getSessions: () => DriftSession[],
): void {
  if (!isSupabaseConfigured() || userId === 'local-guest') return;
  if (driftPushTimer) clearTimeout(driftPushTimer);
  driftPushTimer = setTimeout(() => {
    void pushDriftSessions(userId, getSessions());
  }, 1200);
}
