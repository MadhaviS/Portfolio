...
import { pomodoroRepository } from '../../apps/pulse/data/pomodoroRepository';
import { driftRepository } from '../../apps/drift/data/driftRepository';
...
export function SignInScreen() {
  ...
  const afterAccount = async (accountId: string, displayName?: string) => {
    ...
    driftRepository.switchUser(accountId);
    router.replace('/pomodoro');
  };
  ...
  const submitReset = async () => {
    ...
    try {
      ...
      if (user?.id) {
        ...
        driftRepository.switchUser(user.id);
      }
      router.replace('/pomodoro');
    } catch (e) {
